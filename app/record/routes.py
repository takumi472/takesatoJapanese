...
def create_record():
    ...
    if request.method == "POST":
        try:
            ...
            student_id = (
                int(student_id_raw)
                if student_id_raw and student_id_raw.strip() != ""
                else None
            )
    ...
    if request.method == "POST":
        try:
            ...
            current_app.logger.error(
                f"Date parsing error in edit_record {record_id}: {request.form.get('lesson_date')}",
                exc_info=True,
            )
            ...
            db.session.rollback()
            current_app.logger.error(
                f"Error updating learning record {record_id}: {e}", exc_info=True
            )
