...
cloudinary.config(
    ...
    or current_app.config.get("CLOUDINARY_CLOUD_NAME"),
    api_key=os.environ.get("CLOUDINARY_API_KEY")
    or current_app.config.get("CLOUDINARY_API_KEY"),
    api_secret=os.environ.get("CLOUDINARY_API_SECRET")
    or current_app.config.get("CLOUDINARY_API_SECRET"),
    secure=True,
)
            ...
            for file in files:
                if file and file.filename.endswith(PDF_EXTENSION):
                    ...
                    new_attach = Attachment(
                        filename=save_name, original_name=original_name, meeting=new_m
                    )
        ...
        try:
            ...
            for aid in delete_ids:
                ...
                if attachment and attachment.meeting_id == meeting.id:
                    ...
                    try:
                        ...
                        full_public_id = (
                            f"{FOLDER_PREFIX}/meeting_attachments/{public_id_no_ext}"
                        )
                        # PDFなどのリソースは通常 'image' として扱われます
                        cloudinary.uploader.destroy(full_public_id)
                    except Exception as ce:
                        current_app.logger.warning(
                            f"Cloudinary deletion failed for {attachment.filename}: {ce}"
                        )
            ...
            for file in files:
                if file and file.filename.endswith(PDF_EXTENSION):
                    ...
                    new_attach = Attachment(
                        filename=save_name, original_name=original_name, meeting=meeting
                    )
                    db.session.add(new_attach)
